(function (factory, window) {
    ...
    L.Control.Searchbox = L.Control.extend({
        options: {
            ...
            collapsed: true,
            width: null,
            iconPath: 'img/search_icon_512.png',
            clearIconPath: 'img/close-button.png',  // cross icon when selected
        ...
        clear: function () {
            this.clearInput();
            this.clearItems();
            this._updateButtonIcon('search'); // change to search icon when cleared
        ...
        _onClick: function () {
          if (this._collapsed) {
                ...
                // When cross is clicked, clear the input and revert icon
        ...
        _onListItemClick: function (item) {
            ...
            this._input.focus();
            this.clearItems(); // Oculta el desplegable después de seleccionar una opción
            //this.hide(); // Colapsa la caja de búsqueda
            this._collapsed = false;
            console.log("SHOULD NOT HIDE!");
            // Change the button icon to the 'cross' icon after selection
            this._updateButtonIcon('clear');
        ...
        _updateButtonIcon: function (type) {
            if (type === 'clear') {
                this._icon.setAttribute('src', this.options.clearIconPath); // Change to cross icon
            } else {
                this._icon.setAttribute('src', this.options.iconPath); // Change back to lens icon
            }
        }
    });
